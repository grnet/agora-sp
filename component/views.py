...
def get_components_for_service_details(request, version):
    response = {}
    ...
    try:
        ...
        serv_det_comp = component_models.ServiceDetailsComponent.objects.filter(service_details_id=service_details)

        for s in serv_det_comp:
            c = component_models.ServiceComponentImplementationDetail.objects.get(id=s.service_component_implementation_detail_id.pk)
            components.append(c.as_json())
        ...
        response["status"] = "404 Not Found"
        response["errors"] = {
            "detail": "The requested service component implementation details were not found"
        }
...
def get_service_component_all(request):
    ...
    response["status"] = "200 OK"
    response["data"] = [s.as_json() for s in service_component if (query == None or query == "") or query in s.name.lower()]
    response["info"] = "service component information"
...
def get_service_component_implementation_details(request, comp_imp_det_uuid):
    ...
    try:
        ...
        response["status"] = "200 OK"
        response["data"] = service_component_imp_det.as_full()
...
def edit_service_details_component_implementation_details(request):
    ...
    if "new_component_implementation_details_uuid" not in comp or comp["new_component_implementation_details_uuid"] is None:
    ...
    if "new_service_id" not in comp or comp["new_service_id"] is None:
        return JsonResponse(helper.get_error_response(strings.NEW_SERVICE_UUID_NOT_PROVIDED, status=strings.REJECTED_406),
                            status=406)

    if "new_service_version" not in comp or comp["new_service_version"] is None:
