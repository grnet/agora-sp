...
Uses the latest current version of [Selenium (3.141.0) Python bindings](https://pypi.org/project/selenium/). Which is set to the [`requirements.txt` file](requirements.txt).
...
To install the required python packages, you need to execute the following command:
```bash
...
These tests are intended to run from Jenkins CI/CD.
